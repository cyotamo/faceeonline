...
(function () {
  ...
  function formatarDataCurta(valorData) {
    const valorSeguro = String(valorData || '').trim();
    const correspondencia = valorSeguro.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (correspondencia) return `${correspondencia[3]}/${correspondencia[2]}/${correspondencia[1].slice(-2)}`;
    const data = new Date(valorSeguro);
    if (Number.isNaN(data.getTime())) return valorSeguro || '—';
  ...
  function renderizar() {
    ...
    const tabela = document.createElement('table'); tabela.className = 'tabela-gestao';
    ...
    denuncias.slice((pagina - 1) * porPagina, pagina * porPagina).forEach(item => {
      ...
      [formatarDataCurta(valor(item, 'data', 'dataSubmissao', 'createdAt', 'timestamp')), valor(item, 'nome') || (valor(item, 'identificacao').toLowerCase() === 'sim' ? 'Não informado' : 'Anónimo'), valor(item, 'tipo', 'tipoDenuncia'), formatarDataCurta(valor(item, 'dataOcorrencia')), valor(item, 'local'), valor(item, 'pessoasEnvolvidas')].forEach(v => tr.appendChild(texto(document.createElement('td'), v)));
      const prova = document.createElement('td'), link = urlSegura(urlProva(item));
      if (link) { const a = document.createElement('a'); a.href = link; a.target = '_blank'; a.rel = 'noopener noreferrer'; a.className = 'pdf-icon'; a.setAttribute('aria-label', 'Abrir PDF'); a.textContent = 'PDF'; prova.appendChild(a); } else texto(prova, 'Sem provas'); tr.appendChild(prova);
      const ver = document.createElement('td'); ver.appendChild(botao('👁', 'credencial-btn-acao', () => abrirDetalhes(idDa(item), item))); ver.firstChild.setAttribute('aria-label', 'Ver detalhes da denúncia'); tr.appendChild(ver);
      const accao = document.createElement('td'); accao.appendChild(botao('Finalizar processo', 'credencial-btn-acao', () => finalizar(idDa(item)))); tr.appendChild(accao); corpo.appendChild(tr);
    });
